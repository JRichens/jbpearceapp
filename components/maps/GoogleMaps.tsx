...
const GoogleMaps = () => {
    ...
    useEffect(() => {
        ...
        if (data) {
            setLandAreas((prevLandAreas) => {
                return prevLandAreas.map((localLandArea) => {
                    const updatedArea = data.find(
                        (area) => area.id === localLandArea.id
                    )
                    if (updatedArea) {
                        return {
                            ...localLandArea,
                            notesRead: updatedArea.notesRead,
                        }
                    }
                    return localLandArea
                })
            })
        }
    }, [data]) // Using functional update pattern with setLandAreas, so we don't need landAreas as dependency
    ...
    const handlePolygonComplete = useCallback(
        async (polygon: google.maps.Polygon) => {
            ...
            setPolygonSTid(
                'ST' + easting.toString() + ' ' + northing.toString()
            )
                                    ...
                                    >
                                        ...
                                        <div className="flex flex-row items-center justify-between">
                                            ...
                                            <Button
                                                ...
                                                onClick={async () => {
                                                    await UpdateLandAreasNotesRead(
                                                        landArea.id
                                                    )
                                                    setCurrentLandArea(landArea)
                                                    const landAreaNote =
                                                        await GetLandAreaNotes(
                                                            landArea.id
                                                        )
                                                    landAreaNote &&
                                                        setNoteDetail(
