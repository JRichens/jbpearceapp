...
const PostcodeFinder = () => {
  ...
  const getAddress = async (suggestionid: string) => {
    try {
      const response = await fetch(
  ...
  // Debounce effect on typing postcode that console logs value after 500ms
  useEffect(() => {
    ...
    // returns a cleanup function that is called on each unrender
    return () => {
      clearTimeout(timer)
    }
  }, [search])
    ...
    <div className="flex flex-col gap-2 w-full">
      <div className="flex flex-row gap-2">
        <div className="flex flex-col gap-2 relative">
          ...
          {suggestionDropdown && searchData.length > 0 && search.length > 2 && (
            <div
              ref={dropdownRef}
            ...
            >
              <ul>
                {searchData &&
                  searchData.map((suggestion: SearchedData) => (
                    <li
                      key={suggestion.id}
                      onClick={() => getAddress(suggestion.id)}
                      className="hover:bg-gray-100"
                    >
                      {/* Do not render Separator if it is the first item */}
                      {suggestion.id !== searchData[0].id && <Separator />}
                      <div className="py-1">
